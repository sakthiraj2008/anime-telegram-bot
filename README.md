...
## Connecting your static HTML/CSS/JS site to MongoDB

Browsers can't talk to MongoDB directly (no driver, and it would expose
your DB credentials to anyone who views source). MongoDB used to offer a
way around this — the Atlas Data API — but **that was discontinued in
September 2025**, so the current, correct approach is a small API in
between. This project already includes one: `api_server.py`, running
alongside the bot on the same Koyeb deployment.

**Endpoints** (replace with your actual Koyeb URL):
- `GET https://<your-app>.koyeb.app/api/animes` — list of `{id, title, poster, banner, year, imdb}` for every anime
- `GET https://<your-app>.koyeb.app/api/animes/<id>` — full anime doc, including `seasons` → `episodes`

CORS is already enabled for all origins, so you can call this from any
domain your site is hosted on.

**In your site's JS**, replace wherever you currently do something like
`fetch('data.json')` with:
...
async function loadAnimeList() {
  ...
  animes.forEach(a => {
    console.log(a.id, a.title, a.poster);
    // render your cards here
  });
}

// Single anime page (e.g. ?id=... in the URL)
async function loadAnimeDetail(animeId) {
  const res = await fetch(`${API_BASE}/api/animes/${animeId}`);
  const anime = await res.json();
  anime.seasons.forEach(season => {
    season.episodes.forEach(ep => {
      console.log(`S${season.season_number}E${ep.episode_number}`, ep.title, ep.video);
    });
  });
}
```

The `video` field will be either a plain URL (if you pasted one in the
bot) or a Telegram `file_id` (if you uploaded the video directly to the
bot). Telegram file_ids aren't public URLs by themselves — if you uploaded
videos as files, you'll need a small server-side route that calls
Telegram's `getFile` API and redirects to the resulting file URL, since
file_ids expire/require your bot token to resolve. If you'd like, I can
add a `/api/video/<file_id>` redirect endpoint for that — just paste
video URLs directly in the bot for now and it'll work with no extra step.
